@extends('layouts.app_three')

@section('content')

    <div class="container">
    <div class="col-sm-6">
        <div class="panel panel-default">
            <!-- Default panel contents -->
            <div class="panel-heading" style="background: #C53762;color: #fff;">
                <h3><i class="fa fa-tag"></i> News  </h3>
            </div>
            <!-- List group -->
            <ul class="list-group">
                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>
            </ul>
            <div class="text-center">
                <a class="btn btn-default " href="{{ route('news') }}" style=" color: #fff;
			     color: #000;
			    margin: 10px;">
                    More</a>
            </div>


        </div>
    </div>


    <div class="col-sm-6">
        <div class="panel panel-default">
            <!-- Default panel contents -->
            <div class="panel-heading" style="background: #80146B;color: #fff;">
                <h3><i class="fa fa-fire"></i> Annoucement  </h3>
            </div>
            <!-- List group -->
            <ul class="list-group">
                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>
            </ul>

            <div class="text-center">
                <a class="btn btn-default " href="{{ route('announcements') }}" style=" color: #fff;
			     color: #000;
			    	margin: 10px;">
                    More</a>
            </div>
        </div>
    </div>


    <div class="col-sm-6">
        <div class="panel panel-default" >
            <!-- Default panel contents -->
            <div class="panel-heading" style="background: #00418F;color: #fff;">
                <h3><i class="fa fa-tasks"></i> Duties  </h3>
            </div>
            <!-- List group -->
            <ul class="list-group">
                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>
            </ul>
            <div class="text-center">
                <a class="btn btn-default " href="{{ route('duties') }}" style=" color: #fff;
				     color: #000;
				    margin: 10px;">
                    More</a>
            </div>

        </div>
    </div>




    <div class="col-sm-6">
        <div class="panel panel-default">
            <!-- Default panel contents -->
            <div class="panel-heading"  style="background: #4C3A91;color: #fff;">
                <h3><i class="fa fa-book"></i> Lessons </h3>
            </div>
            <!-- List group -->
            <ul class="list-group">
                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>

                <a href="#"> <li class="list-group-item">Cras justo odio</li></a>
            </ul>
            <div class="text-center">
                <a class="btn btn-default " href="{{ route('lessons') }}" style=" color: #fff;
			     color: #000;
			    margin: 10px;">
                    More</a>
            </div>
        </div>
    </div>


</div>

@endsection